...
pub fn input_name(l: Location, abi: &Abi) -> String {
    if l.space == e5r_ir::op::Space::Stack {
        return slot_name(l);
    }
    if l.space != e5r_ir::op::Space::Register {
...
pub fn is_stack_argument(l: Location) -> bool {
    l.space == e5r_ir::op::Space::Stack && (l.offset as i64) > 0
}
...
impl<'a> Rebuilder<'a> {
    ...
    pub fn integer(&self, e: Expr, o: &Operand) -> Expr {
        ...
        if self.floats.contains(&l) {
            let size = if l.space == e5r_ir::op::Space::Register {
        ...
        if l.space == e5r_ir::op::Space::Register && self.pointers.contains(&l.offset) {
    ...
    pub fn field_access(&self, address: Option<&Operand>, size: u8) -> Option<Expr> {
        ...
        let base = Expr::Input(
            Location {
                space: e5r_ir::op::Space::Register,
    ...
    fn pointer_offset(&self, o: &Operand, depth: u32) -> Option<Walk> {
        ...
        match o {
            Operand::Undefined(l) if l.space == e5r_ir::op::Space::Register => Some(Walk {
                register: l.offset,
                offset: 0,
                index: None,
            }),
    ...
    pub fn definition_site(&self, v: Value) -> Option<(e5r_core::Addr, usize)> {
    ...
    fn narrow_float(&self, op: &SsaOp) -> bool {
        ...
        self.floats.contains(&l)
            && l.space == e5r_ir::op::Space::Register
    ...
    match prototype.filter(|p| !p.parameters.is_empty()) {
        ...
        None => {
            ...
            for l in undefined(f) {
                ...
                let slot = if l.space == e5r_ir::op::Space::Register {
...
fn home_of(l: Location) -> Home {
    match l.space {
        e5r_ir::op::Space::Register => Home::Register(l.offset),
        e5r_ir::op::Space::Stack => Home::Stack(l.offset as i64),
        _ => Home::Anywhere,
    }
}
