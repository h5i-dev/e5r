...
use e5r_core::Addr;
use e5r_diff::{Edit, EditKind, FunctionDiff};
...
fn function_diff(old: &Program, new: &Program, name: &str) -> Option<FunctionDiff> {
    ...
    let map = e5r_diff::mapping(&d);
    e5r_diff::compare_function(old, entry(old, name)?, new, entry(new, name)?, &map)
}
...
fn without_the_function_map_a_moved_callee_is_not_claimed_to_be_displaced() {
    ...
    let d = e5r_diff::compare_function(
        &a,
...
fn the_detail_pass_covers_every_function_the_summary_flagged() {
    ...
    let d = e5r_diff::compare(&a, &b);
    let detail = e5r_diff::detail(&a, &b, &d);
...
fn the_answer_does_not_depend_on_which_run_produced_it() {
    ...
    let first = e5r_diff::detail(&a, &b, &e5r_diff::compare(&a, &b));
    for _ in 0..4 {
        let again = e5r_diff::detail(&a, &b, &e5r_diff::compare(&a, &b));
...
fn an_optimization_level_change_is_reported_without_blowing_up() {
    ...
    let d = e5r_diff::compare(&a, &b);
    for f in e5r_diff::detail(&a, &b, &d) {
        assert!(
