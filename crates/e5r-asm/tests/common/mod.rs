...
/// still e5r's own; this says where one instruction ends and the next starts.
...
pub fn decode_for(arch: &Arch, bytes: &[u8], addr: Addr) -> Option<Insn> {
    e5r_arch::decode(arch, bytes, addr)
}

/// The text the assembler has to accept, which is what `e5r disas` prints.
pub fn render(arch: &Arch, i: &Insn) -> String {
    e5r_arch::format(arch, i, false)
}
...
pub fn normalize(i: &Insn) -> Insn {
    let mut c = *i;
    c.len = 0;
    c.flow = e5r_arch::insn::Flow::Next;
    c
}
