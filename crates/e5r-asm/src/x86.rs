...
use e5r_arch::insn::{AddrMode, Insn, Mem, Operand, Reg, RegClass, Width};
use e5r_core::Addr;
    ...
    fn modrm_mem(&mut self, i: &Insn, reg_field: u8, m: &Mem) -> Result<(), AsmError> {
        ...
        let index = match m.index {
            Some((r, ext, shift)) => {
                if !matches!(
                    ext,
                    e5r_arch::insn::Extend::Lsl | e5r_arch::insn::Extend::LslZero
                ) {
...
mod tests {
    ...
    fn asm_at(text: &str, at: u64) -> Vec<u8> {
        crate::assemble(&e5r_core::Arch::X86_64, text, Addr(at))
    ...
    fn refused(text: &str) -> AsmError {
        crate::assemble(&e5r_core::Arch::X86_64, text, Addr(0x1000)).expect_err("should refuse")
    }
    ...
    fn an_unreachable_branch_names_its_range() {
        let e = crate::assemble(&e5r_core::Arch::X86_64, "call 0x90000000", Addr(0))
            .expect_err("out of range");
        assert!(matches!(e, AsmError::BranchRange { .. }), "{e}");
        // A truncated displacement is the one mistake this must never make.
        assert!(crate::assemble(&e5r_core::Arch::X86_64, "call 0x7ffffffc", Addr(0)).is_ok());
    }
    ...
    fn padding_is_made_of_canonical_no_ops() {
        for n in 1..64usize {
            ...
            let mut at = 0usize;
            while at < p.len() {
                let d = e5r_arch::x86::decode(&p[at..], Addr(0x1000 + at as u64))
