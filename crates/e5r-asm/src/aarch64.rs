...
//! checked against the decoder in `e5r-arch`, which is the inverse function.
...
use e5r_arch::insn::{AddrMode, Extend, Insn, Operand, Reg, RegClass, Shift, Width};
use e5r_core::Addr;
...
mod tests {
    ...
    fn asm(text: &str, at: u64) -> u32 {
        let e = crate::assemble(&e5r_core::Arch::AArch64, text, Addr(at))
    ...
    fn every_case_decodes_to_what_was_written() {
        for (text, _) in CASES {
            ...
            let back = e5r_arch::aarch64::decode(&bytes, Addr(0x1000))
                .unwrap_or_else(|| panic!("{text} does not decode"));
            let printed =
                e5r_arch::aarch64::format(&back, e5r_arch::aarch64::text::Style { objdump: false });
    ...
    fn a_target_the_encoding_cannot_reach_is_refused() {
        for text in [
            ...
            let e = crate::assemble(&e5r_core::Arch::AArch64, text, Addr(0)).expect_err(text);
            assert!(matches!(e, AsmError::BranchRange { .. }), "{text}: {e}");
        }
        // An unaligned target is its own error, not a rounded displacement.
        let e = crate::assemble(&e5r_core::Arch::AArch64, "b\t0x1002", Addr(0x1000))
    ...
    fn a_value_no_immediate_can_build_is_refused_rather_than_approximated() {
        let e = crate::assemble(&e5r_core::Arch::AArch64, "mov\tx0, #0x123456789", Addr(0))
            .expect_err("no single immediate builds this");
        assert!(matches!(e, AsmError::NoEncoding { .. }), "{e}");
        let e = crate::assemble(&e5r_core::Arch::AArch64, "and\tx0, x1, #0x3", Addr(0));
        assert!(e.is_ok(), "a rotated run of ones does encode");
        let e = crate::assemble(&e5r_core::Arch::AArch64, "and\tx0, x1, #0x5", Addr(0))
