...
//! tree is read too when one is on the machine: set `R12E_SLA_DIR` to a
...
use r12e_sla::{Inconsistency, Sla};
...
fn the_minimal_fixture_reads_the_way_its_source_says() {
    ...
    let sp = p
        ...
        .expect("sp exists");
    match sp.body {
        r12e_sla::SymbolBody::Varnode { offset, size, .. } => {
...
fn the_token_fixture_reads_its_fields_and_pcode() {
    ...
    let rd = p
        ...
        .unwrap();
    match &rd.body {
        r12e_sla::SymbolBody::VarnodeList { entries, .. } => assert_eq!(entries.len(), 16),
    ...
    let ctors = match &table.body {
        r12e_sla::SymbolBody::Subtable { constructors, .. } => constructors,
        _ => unreachable!(),
    };
