//! The other direction: a compiled `.sla` back into the model `r12e-sleigh`
//! decodes from.
//!
//! This exists to make the writer's gate a decode rather than a comparison.
//! `r12e-sleigh` parses a `.slaspec` into a [`sl::Spec`] and its engine decodes
...
impl<'a> Load<'a> {
    ...
    fn constructor(
        ...
        sl::Constructor {
            ...
            body: Some(Vec::new()),
            location: r12e_sleigh::Location::new(file, c.line as u32),
        }
    }
