...
use r12e_core::{Addr, Arch, Caps, Evidence, Strength};
use r12e_format::pdata::{ScopeHandler, UnwindOp};
use r12e_format::{LoadOptions, Object, pe};
...
fn render(op: &UnwindOp, frame_register: Option<u8>, frame_offset: u32) -> String {
    fn reg(n: u8) -> String {
        r12e_format::pdata::x64_register(n).to_uppercase()
    }
    match op {
...
fn x64_unwind_codes_match_llvm_readobj() {
    for name in ["win.x64.O0.exe", "win.x64.O2.exe"] {
        ...
        for (block, info) in blocks.iter().zip(&win.exceptions.unwind) {
            ...
            let got_frame = info
                .frame_register
                .map(|r| r12e_format::pdata::x64_register(r).to_uppercase());
...
fn an_unwind_prologue_is_decoded_in_order() {
    ...
    assert_eq!(
        info.frame_register.map(r12e_format::pdata::x64_register),
        Some("rbp")
    );
...
fn an_image_view_translates_rvas_addresses_and_file_offsets() {
    ...
    assert_eq!(r12e_format::windirs::reloc_kind(10), "dir64");
}
