...
fn assemble(text: &str) -> Option<[u8; 4]> {
    let dir = std::env::temp_dir().join(format!("r12e-rev-{}", std::process::id()));
...
fn on_hardware(text: &str, inputs: &[u64]) -> Option<Vec<u64>> {
    let dir = std::env::temp_dir().join(format!("r12e-revrun-{}", std::process::id()));
    std::fs::create_dir_all(&dir).ok()?;
...
fn interpreted(bytes: &[u8; 4], input: u64) -> Option<u64> {
    let at = Addr(0x1000);
    let insn = r12e_arch::aarch64::decode(bytes, at)?;
    let lifted = lift::aarch64::lift(&insn);
    assert!(
        lifted.complete,
        "{} is not modelled, so there is nothing to check",
        r12e_arch::format(&Arch::AArch64, &insn, false)
    );
    let mut m = Machine::new();
    let x0 = r12e_ir::lift::aarch64::gpr_offset(0);
    let x1 = r12e_ir::lift::aarch64::gpr_offset(1);
