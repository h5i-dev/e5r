...
//!
//! The DWARF register numbers are translated through `r12e_ir::dwreg`, which
...
use r12e_format::dwarf::Location as DwarfLocation;
use r12e_format::{Format, LoadOptions};
use r12e_ir::abi::Abi;
use r12e_ir::conv::{self, Convention, Observed};
use r12e_ir::dwreg;
...
fn abi_of(p: &Program) -> Abi {
    r12e_ir::abi::of_container(&p.object.arch, p.object.format == Format::Pe)
}
...
fn ssa(p: &Program) -> BTreeMap<Addr, SsaFunction> {
    ...
    for f in p.functions_by_address() {
        ...
        let mut built = r12e_ir::ssa::build(&ir);
        r12e_ir::opt::optimize(&mut built);
