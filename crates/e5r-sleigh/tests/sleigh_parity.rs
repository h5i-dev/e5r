...
//!
//! 2. **Against the hand-written decoders.** e5r's AArch64 and x86-64
...
//!
//! 3. **On an architecture e5r cannot otherwise decode at all.** RISC-V, built
...
use e5r_sleigh::{DecodeError, Decoded, Decoder, Spec};
...
/// `crates/e5r-arch/tests/objdump_parity.rs` does the same thing for the same
...
/// the same divergence `crates/e5r-arch/tests/objdump_parity.rs` records, and
...
fn aarch64_agrees_with_the_hand_written_decoder() {
    ...
    for path in files(&dir, |n| n.contains(".a64.") && !n.ends_with(".out")) {
        ...
        for line in &lines {
            ...
            let hand = e5r_arch::aarch64::decode(&line.bytes, e5r_core::Addr(line.addr));
            match (&sleigh, hand) {
                (Some(s), Some(h)) => {
                    let ours = s.text(&spec);
                    let theirs = e5r_arch::aarch64::format(
                        &h,
                        e5r_arch::aarch64::text::Style { objdump: true },
                    );
...
/// A new architecture: RISC-V, which e5r has no decoder for at all.
///
...
fn x86_64_agrees_with_the_hand_written_decoder_on_boundaries() {
    ...
    for path in files(&dir, |n| n.contains(".x64.") && n.ends_with(".o")) {
        ...
        for line in &lines {
            let sleigh = d.decode(&line.bytes, line.addr).ok();
            let hand = e5r_arch::x86::decode(&line.bytes, e5r_core::Addr(line.addr));
            match (&sleigh, hand) {
                (Some(s), Some(h)) => {
