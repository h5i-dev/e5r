...
//! Point either at a tree with `E5R_GHIDRA=/path/to/ghidra`.

use e5r_sleigh::model::Approximation;
...
fn out_of_order(spec: &e5r_sleigh::Spec) -> usize {
    let mut bad = 0;
...
fn parse_one(path: &Path) -> Outcome {
    ...
    let result = e5r_sleigh::parse_file(path)
        .map(|spec| Stats {
            ...
            unbound: spec
                ...
                .iter()
                .flat_map(|c| &c.operands)
                .filter(|o| o.source == e5r_sleigh::model::OperandSource::Unbound)
                .count(),
...
fn a_toy_specification_from_the_manual_parses() {
    ...
    let spec = e5r_sleigh::parse_file(&path).expect("the toy specification parses");
    assert_eq!(spec.endian, e5r_sleigh::model::Endian::Big);
...
fn gate() {
    let Some(root) = ghidra_root() else {
        eprintln!("no Ghidra tree found; set E5R_GHIDRA to run the corpus gate");
        return;
    };
...
fn report() {
    let Some(root) = ghidra_root() else {
        eprintln!("no Ghidra tree found; set E5R_GHIDRA to run the sweep");
        return;
    };
