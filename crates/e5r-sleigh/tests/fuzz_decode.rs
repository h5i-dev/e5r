...
//! reproducible from the seed printed in the message. `crates/e5r-sleigh/
...
use e5r_sleigh::{DecodeLimits, Decoder, Spec, pcode};
...
fn toy() -> Spec {
    e5r_sleigh::parse_str(TOY).expect("the toy specification parses")
}
...
fn a_corrupted_specification_does_not_produce_a_misbehaving_decoder() {
    ...
    for round in 0..3_000u64 {
        ...
        let Ok(spec) = e5r_sleigh::parse_str(&text) else {
            continue;
        };
        parsed += 1;
...
"#,
    ...
    assert_eq!(err, e5r_sleigh::DecodeError::TooDeep);
}
...
fn the_published_specifications_survive_noise() {
    ...
    for rel in [
        ...
        let spec = e5r_sleigh::parse_file(&p).expect("the specification parses");
        let mut d = Decoder::new(&spec);
        let mut at = 0usize;
        let mut steps = 0usize;
        let start = Instant::now();
...
fn a_specification_with_no_instruction_table_is_refused() {
    ...
    assert_eq!(
        ...
        e5r_sleigh::DecodeError::NoRootTable
    );
    let _ = Path::new("");
}
