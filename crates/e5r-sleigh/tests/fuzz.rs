...
fn seeds() -> Vec<String> {
    ...
    if let Some(root) = ghidra_root() {
        for name in [
            ...
            if e5r_sleigh::parse_str(&text).is_ok() {
                out.push(text);
            }
        }
    }
...
fn check(text: &str, what: &str) {
    ...
    let _ = e5r_sleigh::parse_str(text);
...
fn the_seeds_themselves_parse() {
    for (i, seed) in seeds().iter().enumerate() {
        let spec = e5r_sleigh::parse_str(seed)
...
fn hostile_shapes_are_refused_rather_than_survived() {
    ...
    for (text, what) in cases {
        let started = Instant::now();
        let result = e5r_sleigh::parse_str(&text);
        let took = started.elapsed();
...
fn an_include_cycle_stops_at_the_depth_limit() {
    let mut loader = e5r_sleigh::preprocess::MemoryLoader::new();
    loader.insert("a.slaspec", "@include \"b.sinc\"\n");
    loader.insert("b.sinc", "@include \"a.slaspec\"\n");
    let started = Instant::now();
    let result = e5r_sleigh::parse_with(
        Path::new("a.slaspec"),
        &mut loader,
        e5r_sleigh::Limits::default(),
        &[],
    );
