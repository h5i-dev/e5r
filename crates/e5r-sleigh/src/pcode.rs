...
//!
//! # The opcode set is e5r's, not a new one
//!
//! [`Opcode`] is variant for variant the same list as `e5r_ir::op::Op`, with
...
//! someone else's specification files, and `e5r-ir` pulls in a serialisation
...
///
/// The same four spaces `e5r_ir::op::Space` has, minus `Stack`, which is
...
/// What an operation does. Variant for variant `e5r_ir::op::Op`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
#[allow(missing_docs)]
pub enum Opcode {
    Copy,
