...
use r12e_sleigh::model::{DisplayPiece, Endian, Offset, OperandSource, Symbol};
...
fn toy() -> r12e_sleigh::Spec {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/data/toy.slaspec");
    r12e_sleigh::parse_file(&path).expect("the toy specification parses")
}
...
fn an_attachment_turns_a_field_into_registers() {
    ...
    let r12e_sleigh::model::Attach::Variables(regs) = attach else {
...
fn aarch64_matches_the_encodings_the_manual_fixes() {
    let Some(root) = ghidra_root() else {
        eprintln!("no Ghidra tree found; set R12E_GHIDRA to run this");
        return;
    };
    ...
    let spec = r12e_sleigh::parse_file(&path).expect("AARCH64 parses");
...
fn powerpc_matches_big_endian_encodings() {
    ...
    let spec = r12e_sleigh::parse_file(&path).expect("ppc_32_be parses");
...
fn table_widths_settle_even_when_tables_recurse() {
    ...
    if fixed.is_file() {
        ...
        let spec = r12e_sleigh::parse_file(&fixed).expect("AARCH64 parses");
    ...
    if varied.is_file() {
        let spec = r12e_sleigh::parse_file(&varied).expect("x86-64 parses");
        let table = spec.table(spec.root());
        assert!(
...
fn a_fixed_width_subtable_places_what_follows_it() {
    ...
    "#;
    let spec = r12e_sleigh::parse_str(source).expect("parses");
    ...
    assert_eq!(
        ...
        Some(r12e_sleigh::model::Approximation::UnknownTokenOffset),
...
fn x86_places_an_immediate_after_the_modrm_it_follows() {
    ...
    let spec = r12e_sleigh::parse_file(&path).expect("x86-64 parses");
...
fn position(c: &r12e_sleigh::model::Constructor, operand: u16) -> usize {
    c.order
        .iter()
...
fn real_encodings_land_in_the_right_bits() {
    ...
    for (spec_path, words) in cases {
        ...
        let spec = r12e_sleigh::parse_file(&path).unwrap_or_else(|e| panic!("{spec_path}: {e}"));
