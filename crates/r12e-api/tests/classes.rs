...
use r12e_api::classes::{Abi, Basis, Class, Role, Rtti, TypeInfoKind};
...
fn open(name: &str) -> Option<Program> {
    ...
    let mut obj = r12e_format::load(&data, &LoadOptions::default()).ok()?;
    ...
    r12e_api::apply_relative_relocations(&mut obj);
...
fn quick() -> r12e_api::classes::Options {
    r12e_api::classes::Options {
        stores: true,
        fields: false,
    }
}
...
fn the_hierarchy_is_the_one_the_source_declares() {
    ...
    for fixture in WITH_RTTI {
        ...
        ran += 1;
        let found = r12e_api::classes_with(&p, &quick());
        assert_eq!(
            found.rtti,
            Rtti::Present,
...
/// promotion in `r12e-ir` does not follow a reload past a call.
#[test]
...
fn the_rtti_build_recovers_strictly_more_than_the_one_without() {
    for (with, without) in WITH_RTTI.iter().zip(WITHOUT_RTTI.iter()) {
        ...
        let rtti = r12e_api::classes_with(&a, &quick());
        let none = r12e_api::classes_with(&b, &quick());
        ...
        let named = |c: &r12e_api::Classes| c.classes.iter().filter(|x| x.name.is_some()).count();
        assert!(
...
fn the_class_names_are_the_ones_cxxfilt_gives() {
    ...
    for fixture in WITH_RTTI {
        ...
        let Some(p) = open(fixture) else { continue };
        let found: BTreeSet<String> = r12e_api::classes_with(&p, &quick())
            .classes
            .iter()
...
fn what_the_shapes_find_is_what_the_names_prove() {
    ...
    for (named, stripped, least) in [
        ...
        ran += 1;
        let proven = r12e_api::classes_with(&a, &quick());
        let inferred = r12e_api::classes_with(&b, &quick());
...
fn nothing_inferred_from_a_real_library_contradicts_its_symbols() {
    ...
    let Ok(mut obj) = r12e_format::load(&data, &LoadOptions::default()) else {
        return;
    };
    ...
    assert!(r12e_api::apply_relative_relocations(&mut obj) > 0);
    let p = analyze(obj, &Options::default());

    let named = r12e_api::classes_with(&p, &quick());
    ...
    let mut bare = analyze(
        r12e_format::load(&data, &LoadOptions::default()).unwrap(),
        &Options::default(),
    );
    assert!(r12e_api::apply_relative_relocations(&mut bare.object) > 0);
    bare.object.symbols.clear();
    let stripped = r12e_api::classes_with(&bare, &quick());
