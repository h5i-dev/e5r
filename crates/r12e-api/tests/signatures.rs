...
fn a_stripped_binary_gets_its_names_back() {
    ...
    let library = r12e_api::collect_signatures(&named, "hello.a64.O2");
    ...
    let found = r12e_api::identify(&stripped, &library);
...
fn a_library_built_from_one_build_recognizes_another() {
    ...
    let library = r12e_api::collect_signatures(&a, "hello.a64.O0");
    let truth: BTreeMap<u64, String> = b
        .functions_by_address()
        .filter_map(|f| Some((f.entry.get(), f.name.clone()?)))
        .collect();
    let found = r12e_api::identify(&b, &library);
    for identified in &found {
