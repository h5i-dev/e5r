...
fn decompile(p: &Program, name: &str, declaration: Option<&str>) -> r12e_api::Decompiled {
    ...
    r12e_api::decompile::decompile_function_with(p, f, &declared)
}
...
fn unit(p: &Program, name: &str, declaration: Option<&str>) -> String {
    ...
    r12e_api::decompile::decompile_program_with(p, &[f], &declared).text()
}
...
fn a_declared_field_name_beats_the_one_the_offset_implies() {
    ...
    let prototype = r12e_decomp::Prototype {
        parameters: vec![r12e_decomp::Param {
            name: "p".to_string(),
    ...
    let mut r = r12e_decomp::expr::Rebuilder::new(&ssa);
    r.declare(&prototype);
