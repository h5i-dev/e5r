...
use r12e_core::{Addr, Bits, Endian, Strength};
use r12e_ir::op::{Op, Space};
use r12e_ir::ssa::{Location, Operand, SsaFunction, SsaKind, Value};
    ...
    for section in &p.object.sections {
        ...
        while at.get() + 3 * w as u64 <= section.range.end().get() {
            ...
            out.push(VTable {
                ...
                evidence: r12e_core::Evidence::DataPointer,
            });
        }
    }
    out
...
fn demangle_type_name(mangled: &str) -> Option<String> {
    let (_, text) = r12e_types::demangle(&format!("_ZTI{mangled}"))?;
        ...
        for (slot, target) in t.methods.iter().enumerate() {
            ...
            found.insert(
                ...
                Member {
                    addr: *target,
                    class: name.clone(),
                    name: p.name_of(*target).map(|n| r12e_types::pretty(&n)),
                    role: Role::Virtual,
                    slot: Some(slot),
                ...
                None => {
                    found.insert(
                        ...
                        Member {
                            addr: at,
                            class: name,
                            name: p.name_of(at).map(|n| r12e_types::pretty(&n)),
                            role,
                            slot,
...
fn member_from_symbol(symbol: &str, addr: Addr) -> Option<Member> {
    let (r12e_types::Scheme::Itanium, text) = r12e_types::demangle(symbol)? else {
        return None;
    };
