...
pub fn callers_of_name<'a>(p: &'a Program, name: &str) -> Vec<&'a Function> {
    ...
    for target in p.functions_by_address() {
        ...
        for x in p.xrefs.to(target.entry) {
            if x.kind != r12e_analysis::XrefKind::Call {
                continue;
            }
...
impl<'a> Analysis<'a> {
    fn new(p: &'a Program) -> Analysis<'a> {
        Analysis {
            p,
            abi: r12e_ir::abi::of(&p.object.arch),
            arity: BTreeMap::new(),
            only: None,
        }
    }
    ...
    fn declared_arity(&mut self, target: Addr) -> usize {
        ...
        let n = self
            ...
            .map(|ssa| r12e_ir::proto::recover(&ssa, &self.abi).integer_arguments)
            .unwrap_or(0);
    ...
    fn judge(&self, site: &Site<'_>, operand: Operand) -> Judgement {
        ...
        let interval = r12e_ir::range::of(&operand, site.ranges);
...
impl Site<'_> {
    ...
    fn edge(&self, b: &r12e_ir::ssa::SsaBlock, cbranch: &SsaOp) -> Option<bool> {
