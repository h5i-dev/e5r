...
//! the same parser is behind `e5r query`, where an agent can put anything it
...
fn asking_about_one_callee_does_not_lift_the_whole_image() {
    ...
    let Ok(obj) = e5r_format::load(&data, &LoadOptions::default()) else {
        return;
    };
    let p = analyze(obj, &Options::default());

    let callers = e5r_api::dataflow::callers_of_name(&p, "memcpy").len();
    assert!(
