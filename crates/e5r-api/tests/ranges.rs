...
fn returned(
    ssa: &SsaFunction,
    abi: &e5r_ir::abi::Abi,
    ranges: &BTreeMap<e5r_ir::ssa::Value, Range>,
) -> Option<Range> {
...
fn every_range_contains_what_the_processor_produced() {
    ...
    for fixture in ["wide.a64.O0.o", "wide.a64.O2.o", "wide.x64.O0.o"] {
        ...
        for case in &cases {
            ...
            let outcome = e5r_api::emulate::run(&p, f, &setup);
            if outcome.stop != e5r_ir::Stop::Returned {
                continue;
            }
            checked += 1;
            if !range.is_any() {
