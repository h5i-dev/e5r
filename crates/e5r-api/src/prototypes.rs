...
pub fn prototypes(p: &Program) -> BTreeMap<Addr, Recovered> {
    ...
    for f in p.functions_by_address() {
        ...
        let prototype = e5r_ir::proto::recover(&built, &abi);
        out.insert(
            f.entry,
            Recovered {
                prototype,
    ...
    for function in ssa.values() {
        ...
        for b in function.blocks.values() {
            for op in &b.ops {
                if op.kind != SsaKind::Op(e5r_ir::op::Op::Call) {
                    continue;
                }
...
fn consumed(f: &SsaFunction, abi: &e5r_ir::abi::Abi) -> BTreeSet<e5r_ir::ssa::Value> {
    ...
    for b in f.blocks.values() {
        ...
        let mut latest: BTreeMap<u64, e5r_ir::ssa::Value> = BTreeMap::new();
        for op in &b.ops {
            match op.kind {
...
fn build(p: &Program, f: &e5r_analysis::Function) -> Option<SsaFunction> {
...
pub fn callers_of(p: &Program, target: Addr) -> BTreeSet<Addr> {
    p.xrefs
        .to(target)
        .iter()
        .filter(|x| x.kind == e5r_analysis::XrefKind::Call)
