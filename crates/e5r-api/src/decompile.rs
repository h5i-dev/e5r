...
use e5r_decomp::{Callee, Output, Param, Prototype};
use e5r_ir::proto::Asserted;
use e5r_ir::ssa::SsaFunction;
use e5r_types::ctype::{Type, TypeId, Types};

pub use e5r_decomp::expr::{Home, Role, Variable};
...
fn one(
    ...
    let switches: e5r_decomp::Switches = f
        .cfg
        .tables
        .iter()
        .filter_map(|t| {
...
fn prototype(
    ...
    if let Some(a) = asserted(p, f, declarations) {
        ...
        let recovered = e5r_ir::proto::recover_with(ssa, &abi, Some(&a));
        return Shape {
...
fn asserted(p: &Program, f: &Function, declarations: &BTreeMap<Addr, String>) -> Option<Asserted> {
    ...
    types.set_model(e5r_ir::proto::model_of(&p.object.arch));
    Asserted::parse_into(types, text).ok()
}

/// The emitter's form of an asserted declaration, laid over the convention.
fn asserted_prototype(a: &Asserted, recovered: &e5r_ir::proto::Prototype) -> Prototype {
    let types = &a.types;
...
fn recovered(p: &Program, f: &Function, ssa: &SsaFunction) -> Option<Prototype> {
    ...
    for n in 0..recovered.integer_arguments {
        ...
        let tag = format!("{}_{name}", e5r_decomp::identifier(&f.display_name()));
        ...
        let (decl, pointer) = match (fields.is_empty(), single) {
            ...
            (true, Some(size)) => (format!("{} *{name}", e5r_decomp::c_type(size)), true),
            (true, None) => (format!("{} {name}", e5r_decomp::c_type(width)), false),
        };
        parameters.push(Param {
            decl,
            name,
    ...
    for offset in &recovered.stack_arguments {
        let name = e5r_decomp::slot_name(e5r_ir::ssa::Location {
            space: e5r_ir::op::Space::Stack,
...
fn structure(name: &str, fields: &[(i64, u8)]) -> String {
    ...
    for (offset, size) in fields {
        ...
        out.push_str(&format!(" {ty} {};", e5r_decomp::field_name(*offset)));
