...
use e5r_analysis::{Function, JumpTable, Program};
use e5r_core::{Addr, Arch, MemoryMap};
use e5r_ir::{Machine, Op, Stop};
...
pub fn run(p: &Program, f: &Function, setup: &Setup) -> Run {
    ...
    match arch {
        ...
        _ => m.set_reg(e5r_ir::lift::aarch64::gpr_offset(30), 8, SENTINEL),
    }
...
fn walk(
    ...
    loop {
        ...
        let Some(insn) = e5r_ir::func::decode_at(image, arch, pc) else {
            w.ending = Ending::NoCode(pc);
            return w;
        };
        let lifted = e5r_ir::lift::lift(arch, &insn);
        if !lifted.complete {
            w.unlifted.push(pc);
        }
