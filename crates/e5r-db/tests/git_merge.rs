...
fn setup(dir: &Path) -> Option<()> {
    ...
    std::fs::write(dir.join(".gitattributes"), "*.e5r merge=union\n").ok()?;
    Some(())
}

#[test]
fn two_branches_merge_without_conflict_markers() {
    let tmp = std::env::temp_dir().join(format!("e5r-merge-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).unwrap();
    if setup(&tmp).is_none() {
        return; // no git here
    }
    let file = tmp.join("bin.e5r");
...
fn editing_the_same_function_resolves_rather_than_conflicting() {
    let tmp = std::env::temp_dir().join(format!("e5r-same-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).unwrap();
    if setup(&tmp).is_none() {
        return;
...
fn the_file_survives_a_round_trip_through_git() {
    let tmp = std::env::temp_dir().join(format!("e5r-rt-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).unwrap();
    if setup(&tmp).is_none() {
        return;
    ...
    git(&tmp, &["checkout", "-q", "--", "bin.e5r"]).unwrap();
