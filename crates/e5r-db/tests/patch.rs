...
use e5r_core::Addr;
use e5r_db::anchor::{Anchor, AnchorIndex, Resolution};
use e5r_db::patch::{Conflict, Edit, Patch};
...
fn function(base: u64, displacement: u32) -> (Anchor, Vec<u8>) {
    ...
    let insns: Vec<e5r_arch::Insn> = words
        .iter()
        .enumerate()
        .filter_map(|(n, w)| e5r_arch::aarch64::decode_word(*w, Addr(base + n as u64 * 4)))
        .collect();
