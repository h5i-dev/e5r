...
use e5r_db::patch::Patch;
use e5r_db::project::{Project, Verdict, digest};
...
fn a_session_round_trips_through_the_file() {
    ...
    assert_eq!(back.log.as_deref(), Some("notes.e5r"));
    assert_eq!(back.signatures.len(), 2);
    assert_eq!(back.patches, ["nop-check.e5r-patch"]);
...
fn a_project_for_a_real_binary_recognizes_it() {
    ...
    p.patches.push("ident.e5r-patch".into());
