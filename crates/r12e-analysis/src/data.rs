...
//! `r12e_format::Object` does not carry the relocations it applied, so this
//! crate cannot see them.

use std::collections::BTreeMap;

use r12e_arch::{Insn, Operand};
use r12e_core::{Addr, AddrRange, Arch, MemoryMap};
...
fn declared_objects(object: &Object) -> Vec<Region> {
    ...
    object
        ...
        .iter()
        .filter(|s| {
            s.kind == r12e_format::SymbolKind::Object
                && s.size > 0
                && s.addr != Addr::ZERO
...
fn pointer_runs(object: &Object) -> Vec<Region> {
    ...
    let little = object.endian == r12e_core::Endian::Little;
    let mem = &object.memory;
