...
//!
//! `perf` on this machine does not know Rust's v0 scheme, so a profile of e5r
//! reads as `_RNvNtCs..._13e5r_analysis3cfg10build_with`. This crate already
//! depends on the demangler the rest of the tool uses, so the profile can be
//! made readable without installing anything.
//!
//! It belongs in the CLI as `e5r demangle` rather than here. It is an example
...
//!
//! Usage: ... | cargo run --release -p e5r-analysis --example demangle
...
fn rewrite(line: &str, out: &mut String) {
    ...
    while i < b.len() {
        if (b[i] == b'_' || b[i] == b'?') && (i == 0 || !symbolic(b[i - 1])) {
            ...
            if let Some((_, name)) = e5r_types::demangle(stem) {
                out.push_str(&name);
                i = j;
                continue;
            }
