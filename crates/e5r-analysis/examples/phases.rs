...
//!
//! Usage: cargo run --release -p e5r-analysis --example phases -- <binary> [no-scan]

use std::collections::BTreeMap;

use e5r_analysis::program::{Function, Options};
use e5r_analysis::{Cfg, Session};
use e5r_core::Addr;
...
fn account(functions: &BTreeMap<Addr, Function>) {
    ...
    for f in functions.values() {
        ...
        provenance += f.provenance.corroborating.capacity() * size_of::<e5r_core::Evidence>();
        ...
        tables += f.cfg.tables.capacity() * size_of::<e5r_analysis::JumpTable>();
    ...
    let mut table_bytes = distinct * (size_of::<e5r_analysis::Block>() + size_of::<Addr>());
    ...
    println!(
        ...
        kb(table_bytes),
        size_of::<e5r_analysis::Block>() + size_of::<Addr>(),
        kb(succ)
    );
    println!(
    ...
    println!(
        "{insns} instructions are decoded and thrown away; one decoded Insn is {} B, so keeping \
         them would cost {} MB",
        size_of::<e5r_arch::Insn>(),
        insns * size_of::<e5r_arch::Insn>() as u64 / (1024 * 1024)
    );
...
fn main() {
    ...
    // where `e5r` pays only for the pages it touches. Everything after it is
    // the same either way, which is the part being measured.
    let data = std::fs::read(&path).expect("read");
    at = mark("file read", at, format!("{} KB of file", data.len() / 1024));

    let object: Object = e5r_format::load(&data, &LoadOptions::default()).expect("load");
    at = mark(
        "load",
        at,
        format!(
    ...
    println!(
        ...
        max_insns as usize * size_of::<e5r_arch::Insn>() / 1024
    );
    println!(
        "sizeof Function {}, Block {}, Cfg {}, Xref {}, Found {}, JumpTable {}",
        size_of::<Function>(),
        size_of::<e5r_analysis::Block>(),
        size_of::<Cfg>(),
        size_of::<e5r_analysis::Xref>(),
        size_of::<e5r_analysis::Found>(),
        size_of::<e5r_analysis::JumpTable>(),
    );
}
