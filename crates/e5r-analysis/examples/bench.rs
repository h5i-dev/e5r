...
use e5r_analysis::{Cache, ContentHash, Options, Session};
...
fn main() {
    ...
    let object = e5r_format::load(&data, &LoadOptions::default()).expect("load");
    ...
    let note = if mode == "funcs-eager" {
        let p = e5r_analysis::analyze(object, &opts);
