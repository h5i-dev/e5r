...
use e5r_arch::{Flow, Insn};
use e5r_core::{Addr, AddrRange, Arch, Caps, MemoryMap};
...
fn count_insns(mem: &MemoryMap, arch: &Arch, start: Addr, end: Addr) -> u32 {
    ...
    while at < end {
        ...
        let Some(i) = e5r_arch::decode(arch, w, at) else {
            break;
        };
        at = i.next();
        n += 1;
...
fn decode_stateful(
    ...
    window: &[u8],
    at: Addr,
    it: &mut e5r_arch::arm::ItState,
) -> Option<Insn> {
    if *arch == Arch::Thumb {
        let (insn, next) = e5r_arch::arm::decode_thumb(window, at, *it)?;
        *it = next;
        return Some(insn);
    }
    e5r_arch::decode(arch, window, at)
}
