...
use e5r_analysis::{Cache, ContentHash, Options, Session, analyze};

/// A session over `name` that reads and writes `dir`.
fn cached(dir: &Path, name: &str, opts: &Options) -> e5r_analysis::Program {
...
fn the_cache_directory_follows_xdg() {
    ...
    if let Some(dir) = dir {
        ...
        assert_eq!(dir, expected.join("e5r").join("analysis"));
    }
}
