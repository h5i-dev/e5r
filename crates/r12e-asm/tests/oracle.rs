...
//!
//! The round-trip gate proves the encoder agrees with r12e's own decoder,
...
fn position_dependent(arch: &Arch, i: &r12e_arch::insn::Insn) -> bool {
    if matches!(
        i.flow,
        r12e_arch::insn::Flow::Branch(_)
            | r12e_arch::insn::Flow::CondBranch(_)
            | r12e_arch::insn::Flow::Call(_)
    ) {
        return true;
    }
    i.operands().iter().any(|o| match o {
        r12e_arch::insn::Operand::Addr(_) => true,
        r12e_arch::insn::Operand::Mem(m) => m.is_pc_relative(),
        _ => false,
    })
...
fn compare(arch: &Arch, texts: &[(String, Vec<u8>)], dir: &Path, t: &mut Tally) {
    ...
    for (k, ix) in kept.iter().enumerate() {
        ...
        match (mine, yours) {
            ...
            _ => t
                .wrong
                .push(format!("{text:?}: r12e {ours:02x?}, system {theirs:02x?}")),
        }
    }
}
...
fn run(arch: &Arch, tag: &str) {
    ...
    let tmp = std::env::temp_dir().join(format!("r12e-asm-oracle-{tag}"));
...
///
/// `.org` gives the system assembler the same addresses r12e was told, and the
...
fn branch_displacements_match_the_system_assembler() {
    ...
    for (arch, mnemonics, spread) in [
        ...
        let tmp = std::env::temp_dir().join("r12e-asm-oracle-branch");
        let _ = std::fs::create_dir_all(&tmp);
        ...
        for (text, ours, at) in &want {
            ...
            if theirs != ours {
                differ.push(format!(
                    "{text} at {at:#x}: r12e {ours:02x?}, system {theirs:02x?}"
                ));
            }
        }
