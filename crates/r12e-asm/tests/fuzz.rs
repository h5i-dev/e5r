...
//!
//! Modelled on `r12e-format`'s loader fuzz for the same reason: a fuzz target
...
use r12e_asm::{assemble, assemble_all, error::MAX_TEXT, pad, parse};
...
fn mutated_assembly_text_never_panics() {
    ...
    while start.elapsed() < BUDGET {
        ...
        if let Ok(e) = assemble(&arch, &text, at) {
            assert!(!e.is_empty() && e.len() <= r12e_asm::MAX_INSN, "{text:?}");
