...
pub struct Common {
    ...
    #[arg(long)]
    no_scan: bool,
    /// Annotation log to use. Defaults to the binary's path plus `.e5r`.
    #[arg(long, global = true)]
    db: Option<PathBuf>,
...
pub enum Command {
    ...
    Emulate {
        ...
        /// what this was: `e5r emulate` could not be run at all.
...
pub enum ProjectCommand {
    ...
    New {
        ...
        #[arg(long = "set")]
        settings: Vec<String>,
        /// The annotation log. Defaults to the binary's path plus `.e5r`.
        #[arg(long)]
        log: Option<PathBuf>,
    },
...
pub fn run(cli: &Cli, w: &mut out::Out) -> Result<u8, String> {
    ...
    let mut object = e5r_format::load(&data, &load_opts).map_err(|e| e.to_string())?;
    ...
    let program = e5r_analysis::analyze(object, &opts);
...
pub fn dispatch(
    cli: &Cli,
    w: &mut out::Out,
    program: &mut e5r_analysis::Program,
    data: &[u8],
    ...
    match &cli.command {
        ...
        Command::Sig { what, common } => match what {
            SigCommand::Create { out } => {
                let library = e5r_api::collect_signatures(
                    program,
                    &common
                        .file
                        .file_name()
            ...
            SigCommand::Apply { library } => {
                ...
                let library = e5r_db::signature::Library::from_text(&text);
        ...
        Command::Diff { common, other, all } => {
            ...
            let other_obj = e5r_format::load(&other_data, load_opts).map_err(|e| e.to_string())?;
            let other_prog = e5r_analysis::analyze(other_obj, opts);
...
mod tests {
    ...
    /// `e5r emulate` could not be run at all. Nothing noticed, because
