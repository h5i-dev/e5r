...
use e5r_analysis::{Function, Program, Xref};
use e5r_arch::Insn;
use e5r_core::Addr;
...
pub fn info(o: &Object) -> Info {
    Info {
        ...
        endian: match o.endian {
            e5r_core::Endian::Little => "little",
            e5r_core::Endian::Big => "big",
        },
...
fn insn_out(p: &Program, i: &Insn) -> InsnOut {
    InsnOut {
        ...
        flow: match i.flow {
            ...
            e5r_arch::Flow::Branch(_) => "branch",
            e5r_arch::Flow::CondBranch(_) => "cond_branch",
            e5r_arch::Flow::IndirectBranch => "indirect_branch",
            e5r_arch::Flow::Call(_) => "call",
            e5r_arch::Flow::IndirectCall => "indirect_call",
            e5r_arch::Flow::Return => "return",
            e5r_arch::Flow::Trap => "trap",
            e5r_arch::Flow::Syscall => "syscall",
        }
        .to_string(),
...
pub fn decompiled(items: Vec<(&Function, &e5r_api::Decompiled)>) -> Listing<DecompileOut> {
    listing(
        items
            .into_iter()
...
pub fn shapes(found: &[(&Function, Vec<e5r_api::Pointer>)]) -> Listing<ShapeOut> {
    listing(
        found
            .iter()
...
pub fn identified(found: &[e5r_api::Identified]) -> Listing<IdentifiedOut> {
    listing(
        found
            .iter()
            .map(|i| IdentifiedOut {
...
pub fn query(answer: &e5r_api::Answer) -> QueryOut {
    let items = answer
        .rows
        .iter()
        .map(|row| {
...
pub fn run(f: &Function, r: &e5r_api::Run) -> RunOut {
    RunOut {
        function: function_out(f),
...
pub fn vtables(p: &Program, tables: &[e5r_api::VTable]) -> Listing<VTableOut> {
    listing(
        tables
            .iter()
            .map(|t| VTableOut {
...
pub fn diff(d: &e5r_diff::Diff) -> DiffOut {
    let pair = |m: &e5r_diff::Match| {
        json!({
    ...
    let side =
        |(a, n): &(e5r_core::Addr, String)| json!({ "addr": format!("{:#x}", a.get()), "name": n });
    DiffOut {
        schema: SCHEMA,
...
pub fn overlay(r: &e5r_format::overlay::Report) -> OverlayReport {
    OverlayReport {
        schema: SCHEMA,
...
pub fn archive(a: &e5r_format::archive::Archive) -> ArchiveOut {
    ArchiveOut {
        schema: SCHEMA,
...
pub fn classes(c: &e5r_api::classes::Classes) -> ClassesOut {
    ClassesOut {
        schema: SCHEMA,
        abi: c.abi.as_str(),
        rtti: c.rtti.as_str(),
