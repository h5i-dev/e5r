...
pub fn symbols(w: &mut Out, o: &Object, as_json: bool) -> R {
    ...
    for s in &o.symbols {
        outln!(
            ...
            w.paint(Role::Name, &last(w, 48, &e5r_types::pretty(&s.name)))
        );
    }
    Ok(exit::OK)
}
...
pub fn imports(w: &mut Out, o: &Object, as_json: bool) -> R {
    ...
    for i in &o.imports {
        let name = e5r_types::pretty(&i.name);
        let lib = i
            .library
            .as_deref()
            .map(|l| format!("{l}  "))
...
pub fn exports(w: &mut Out, o: &Object, as_json: bool) -> R {
    ...
    for e in &o.exports {
        outln!(w, "{}  {}", e.addr, e5r_types::pretty(&e.name));
    }
    Ok(exit::OK)
}
...
pub fn disas(
    ...
    let mut chosen: Vec<&e5r_analysis::Function> = if target == "all" {
    ...
    for (n, f) in chosen.iter().enumerate() {
        ...
        for i in &insns {
            ...
            let text = e5r_arch::format(&p.object.arch, i, false);
...
fn annotate(p: &Program, i: &e5r_arch::Insn) -> String {
...
pub fn diff(w: &mut Out, old: &Program, new: &Program, all: bool, as_json: bool) -> R {
    let d = e5r_diff::compare(old, new);
    if as_json {
...
pub fn decompile(w: &mut Out, p: &Program, target: &str, as_json: bool, limits: Limits<'_>) -> R {
    ...
    let chosen: Vec<&e5r_analysis::Function> = if target == "all" {
        p.functions_by_address().collect()
    } else {
        let Some(a) = addr::resolve(p, target) else {
            return Err(format!("{target:?} is not an address, a symbol, or `all`"));
    ...
    let mut unit = e5r_api::Unit::default();
    let mut why = Stopped::Finished;
    if budget.unlimited() {
        unit = e5r_api::decompile_program_with(p, &chosen, declared);
        bar.step();
    } else {
        for group in chosen.chunks(CHUNK) {
            let mut taking: Vec<&e5r_analysis::Function> = Vec::with_capacity(group.len());
            for f in group {
                why = budget.take();
                if !why.complete() {
                    break;
            ...
            if !taking.is_empty() {
                let part = e5r_api::decompile_program_with(p, &taking, declared);
...
pub fn shapes(w: &mut Out, p: &Program, target: &str, as_json: bool) -> R {
    ...
    let found: Vec<(&e5r_analysis::Function, Vec<e5r_api::Pointer>)> = chosen
        .iter()
        .map(|f| (*f, e5r_api::shapes_of(p, f)))
...
pub fn identified(
    w: &mut Out,
    p: &Program,
    library: &e5r_db::signature::Library,
    as_json: bool,
) -> R {
    let found = e5r_api::identify(p, library);
    if as_json {
...
pub fn query(w: &mut Out, p: &Program, text: &str, as_json: bool) -> R {
    ...
    let answer = e5r_api::query::run(p, text)?;
    if as_json {
...
pub fn emulate(
    ...
    let setup = e5r_api::Setup {
        arguments: args.to_vec(),
        depth,
        budget,
        ..Default::default()
    };
    let run = e5r_api::emulate::run(p, f, &setup);
...
fn describe(stop: &e5r_ir::Stop) -> String {
    use e5r_ir::Stop;
    match stop {
...
pub fn overlay(w: &mut Out, o: &Object, data: &[u8], as_json: bool) -> R {
    let report = e5r_format::overlay::analyze(o, data);
    if as_json {
...
pub fn archive(w: &mut Out, data: &[u8], symbols: bool, as_json: bool) -> R {
    let ar = e5r_format::archive::open(data).map_err(|e| e.to_string())?;
    if as_json {
...
pub fn classes(w: &mut Out, p: &Program, members: bool, as_json: bool) -> R {
    ...
    let found = e5r_api::classes::classes_with(p, &opts);
    if as_json {
