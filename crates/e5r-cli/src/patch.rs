...
use e5r_core::Addr;
use e5r_db::patch::{Applied, Change, Conflict, Edit, Patch};
use e5r_db::project::{Project, Verdict};
...
fn emit(
    ...
    let tmp = dest.with_extension("e5r-patching");
...
pub fn record(
    ...
    let mut bytes = match replace.source {
        Source::Bytes(b) => b,
        Source::Asm(text) => {
            e5r_asm::assemble_all(&p.object.arch, text, at).map_err(|e| format!("{target}: {e}"))?
        }
    };
    if let Some(n) = replace.pad_to {
        ...
        bytes.extend(e5r_asm::pad(&p.object.arch, n - bytes.len()).map_err(|e| e.to_string())?);
    }
    let replace = &bytes[..];
