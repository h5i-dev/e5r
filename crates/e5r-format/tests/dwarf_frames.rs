...
use e5r_format::dwarf::DebugInfo;
...
fn location_lists_match_readelf() {
    ...
    for name in FIXTURES {
        ...
        for (i, die) in dies.iter().enumerate() {
            ...
            let Some(f) = debug
                .functions
                .get(&e5r_core::Addr(function.wrapping_add(base)))
            else {
                continue;
            };
            ...
            let candidates: Vec<&e5r_format::dwarf::DebugLocal> = f
                .locals
                .iter()
    ...
    for name in FIXTURES {
        ...
        for f in debug.functions.values() {
            for local in &f.locals {
                let mut places: Vec<&e5r_format::dwarf::Location> =
