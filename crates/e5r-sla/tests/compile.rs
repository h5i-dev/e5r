...
//!    what reads back must agree with what `e5r-sleigh` parsed: the same
...
use e5r_sla::{Sla, model::SymbolBody};
...
fn ghidra_roots() -> Vec<PathBuf> {
    ...
    if let Ok(v) = std::env::var("E5R_GHIDRA_DIR") {
...
fn every_slaspec_compiles_and_reads_back() {
    ...
    for path in &specs {
        let name = path
            ...
            .to_string_lossy()
            .into_owned();
        let spec = match e5r_sleigh::parse_file(path) {
            Ok(s) => s,
            Err(e) => {
...
fn agrees(spec: &e5r_sleigh::Spec, back: &Sla, report: &sleighc::Report) -> Result<(), String> {
    let p = &back.program;
    ...
    let declared = spec
        ...
        .filter(|s| {
            !matches!(
                s.kind,
                e5r_sleigh::model::SpaceKind::Constant | e5r_sleigh::model::SpaceKind::Unique
            )
        })
        .count();
    ...
    for s in p.subtables() {
        ...
        for (k, c) in constructors.iter().enumerate() {
            for (i, o) in c.operands.iter().enumerate() {
                ...
                match expr.first() {
                    Some(e5r_sla::model::Expr::OperandValue { table, ct, .. })
...
fn byte_comparison_against_the_reference_compiler() {
    ...
    let specs = slaspecs();
    let tmp = std::env::temp_dir().join("e5r-sleighc");
    ...
    for path in specs.iter() {
        ...
        let Ok(spec) = e5r_sleigh::parse_file(path) else {
            continue;
        };
        ...
        let a = e5r_sla::encode::encode(&ours.tree).expect("encodes");
        let b = e5r_sla::encode::encode(&theirs.tree).expect("encodes");
        if a == b {
            identical += 1;
            continue;
        }
...
fn the_committed_fixtures_compile() {
    ...
    for name in ["minimal", "tokens", "subtable"] {
        ...
        let spec = e5r_sleigh::parse_str(&text).expect("the fixture source parses");
...
/// compiler's output by hand. `E5R_SPEC` names the `.slaspec`, `E5R_OUT` the
/// file to write. A diagnostic, not a gate.
#[test]
#[ignore = "diagnostic: writes one compiled language to E5R_OUT"]
fn write_one_compiled_language() {
    let Ok(spec_path) = std::env::var("E5R_SPEC") else {
        println!("set E5R_SPEC and E5R_OUT");
        return;
    };
    let out = std::env::var("E5R_OUT").unwrap_or_else(|_| "out.sla".into());
    let spec = e5r_sleigh::parse_file(Path::new(&spec_path)).expect("parses");
    let (bytes, report) = sleighc::compile_to_bytes(&spec).expect("compiles");
...
///
/// `e5r-sleigh`'s decode engine is measured at zero disagreements with
...
fn a_file_we_wrote_decodes_the_way_its_source_does() {
    ...
    for (name, rel, width) in langs {
        ...
        any = true;
        let spec = e5r_sleigh::parse_file(&path).expect("the language parses");
        let (bytes, report) = sleighc::compile_to_bytes(&spec).expect("compiles");
        let back = Sla::parse(&bytes).expect("reads back");
...
fn compare_decodes(a: &e5r_sleigh::Spec, b: &e5r_sleigh::Spec, width: usize) -> DecodeParity {
    ...
    let mut da = e5r_sleigh::Decoder::new(a);
    let mut db = e5r_sleigh::Decoder::new(b);
