//! The SLEIGH compiler: an `e5r-sleigh` specification to the model
//! `e5r-sla` writes, and from there to `.sla` bytes.
//!
//! # Why this is in the test harness
//!
//! It should be a module of `e5r-sleigh`, next to the front end whose model
//! it consumes. It is not, because `e5r-sleigh`'s decode engine reads a
//! compiled `.sla` and will therefore depend on `e5r-sla`, and cargo refuses
...
pub fn compile_to_bytes(spec: &sl::Spec) -> Result<(Vec<u8>, Report), e5r_sla::SlaError> {
    let (p, r) = compile(spec);
    let bytes = e5r_sla::emit::write(&p, e5r_sla::KNOWN_VERSION, e5r_sla::Level::Fixed)?;
    Ok((bytes, r))
}
...
impl<'a> Compiler<'a> {
    ...
    fn run(mut self) -> (sla::Program, Report) {
        ...
        p.version = Some(u64::from(e5r_sla::KNOWN_VERSION));
