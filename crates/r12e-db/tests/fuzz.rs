...
use r12e_db::signature::{Library, MAX_SIGNATURES, Signature};
...
fn a_file_longer_than_the_cap_stops_at_a_number() {
    ...
    let mut text = String::from("# r12e signatures v1\n");
    for n in 0..64u64 {
...
fn a_library_full_of_one_name_still_refuses_the_coin_flip() {
    ...
    let a = r12e_db::Anchor {
        shape: 1,
        bytes: 2,
        insns: 40,
        abs: r12e_core::Addr(0x1000),
        offset: 0,
    };
...
fn anchor(s: &Signature) -> r12e_db::Anchor {
    r12e_db::Anchor {
        shape: s.shape,
        bytes: s.bytes,
        insns: s.insns,
