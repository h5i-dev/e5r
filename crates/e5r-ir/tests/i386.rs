...
fn on_hardware() -> Option<Vec<u32>> {
    ...
    let dir = std::env::temp_dir().join(format!("e5r-i386-{}", std::process::id()));
...
fn assemble(text: &str) -> Option<Vec<u8>> {
    let cc = tool("clang")?;
    let dir = std::env::temp_dir().join(format!("e5r-i386asm-{}", std::process::id()));
    std::fs::create_dir_all(&dir).ok()?;
...
fn interpreted(bytes: &[u8]) -> Option<u32> {
    ...
    let mut at = 0usize;
    while at < bytes.len() {
        let insn = e5r_arch::x86::decode32(&bytes[at..], Addr(0x1000 + at as u64))?;
        let lifted = lift::x86::lift32(&insn);
        assert!(
            lifted.complete,
            "{} is not modelled",
            e5r_arch::format(&Arch::X86, &insn, false)
        );
