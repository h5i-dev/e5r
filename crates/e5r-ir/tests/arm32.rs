...
fn on_hardware(cases: &[(&str, &str)], thumb: bool) -> Option<Vec<u32>> {
    ...
    let dir = std::env::temp_dir().join(format!("e5r-arm32-{set}-{}", std::process::id()));
...
fn assemble(text: &str, thumb: bool) -> Option<(Vec<u8>, usize)> {
    let cc = tool("clang")?;
    let set = if thumb { "t32" } else { "a32" };
    let dir = std::env::temp_dir().join(format!("e5r-arm32asm-{set}-{}", std::process::id()));
    std::fs::create_dir_all(&dir).ok()?;
...
fn interpreted(bytes: &[u8], end: usize, thumb: bool) -> Option<u32> {
    ...
    let mut it = e5r_arch::arm::ItState::default();
    while at < end {
        let arch = if thumb { Arch::Thumb } else { Arch::Arm };
        let insn = if thumb {
            let (i, next) = e5r_arch::arm::decode_thumb(&bytes[at..], Addr(BASE + at as u64), it)?;
            it = next;
            i
        } else {
            e5r_arch::arm::decode(&bytes[at..], Addr(BASE + at as u64))?
        };
        let lifted = lift::arm::lift_mode(&insn, thumb);
        assert!(
            lifted.complete,
            "{} is not modelled",
            e5r_arch::format(&arch, &insn, false)
        );
