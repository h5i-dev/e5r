...
fn on_hardware(insn: &str, value: u64) -> Option<Vec<(u64, u64)>> {
    ...
    let dir = std::env::temp_dir().join(format!("e5r-psh-{}", std::process::id()));
...
fn assemble(text: &str) -> Option<Vec<u8>> {
    let cc = tool("clang")?;
    let dir = std::env::temp_dir().join(format!("e5r-pshasm-{}", std::process::id()));
    std::fs::create_dir_all(&dir).ok()?;
...
fn interpreted(bytes: &[u8], value: u64, count: u64) -> Option<(u64, u64)> {
    let insn = e5r_arch::x86::decode(bytes, Addr(0x1000))?;
    let lifted = lift::x86::lift(&insn);
    assert!(
        lifted.complete,
        "{} is not modelled",
        e5r_arch::format(&Arch::X86_64, &insn, false)
    );
