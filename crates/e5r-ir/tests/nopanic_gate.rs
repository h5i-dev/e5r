...
//! not covered.
//!
//! It lives in `e5r-ir` because that is the only crate in the gate list whose
//! dev-dependencies reach both `e5r-format` and `e5r-arch`; a gate split
...
use e5r_arch::decode;
use e5r_core::{Addr, Arch, Caps};
use e5r_format::{LoadOptions, Object, archive, metadata, overlay, pdb, swift};
...
fn drive_every_entry_point(data: &[u8], opts: &LoadOptions) -> Option<Object> {
    ...
    let _ = e5r_format::raw::load(data, &raw_opts);

    let obj = match e5r_format::load(data, opts) {
        Ok(o) => o,
        Err(_) => return None,
    };
    ...
    for s in obj.sections.iter().filter(|s| s.exec).take(8) {
        ...
        while i < bytes.len() {
            ...
            if let Some(insn) = decode(&obj.arch, &bytes[i..], at) {
                ...
                let _ = e5r_arch::format(&obj.arch, &insn, true);
            }
            i += step;
        }
    }
...
fn check_object(obj: &Object, data: &[u8], caps: &Caps, what: &str) {
    ...
    // unchecked: `Object::symbol_at` in `e5r-format/src/lib.rs` and
    // `e5r-api/src/vtables.rs`. That is a real defect with its own
...
fn truncation_at_every_boundary_stops_cleanly() {
    ...
    for seed in &seeds {
        ...
        if let Ok(obj) = e5r_format::load(&seed.data, &opts) {
            for s in &obj.sections {
...
///
/// `e5r-arch` has its own fuzz test per decoder; this one drives them through
    ...
    while started.elapsed() < DECODE_BUDGET {
        ...
        for (k, arch) in arches.iter().enumerate() {
            ...
            if let Some(insn) = decode(arch, &buf[..len], Addr(0x40_0000)) {
                ...
                let _ = e5r_arch::format(arch, &insn, true);
                let _ = e5r_arch::format(arch, &insn, false);
                decoded[k] += 1;
            }
            assert!(
...
fn a_macho_section_off_the_end_is_reported() {
    ...
    let obj = e5r_format::load(&bytes, &LoadOptions::default())
...
///
/// `crates/e5r-format/src/macho.rs` lays sections out with
...
fn a_macho_relocatable_layout_stays_inside_the_address_space() {
    ...
    let obj =
        e5r_format::load(&bytes, &LoadOptions::default()).expect("a 344-byte Mach-O still loads");
...
fn symbol_at_adds_a_file_chosen_size_unchecked() {
    ...
    let Ok(obj) = e5r_format::load(&bad, &opts) else {
