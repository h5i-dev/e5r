"""DecBench backend for r12e, living outside the DecBench checkout.
...
r12e has no Python API, so the backend shells out to the CLI. One
`r12e decompile <binary> all --json` call per binary is far cheaper than one
...
Set R12E_MODE=targets to invoke once per requested address instead, which is
...
def r12e_binary() -> Path | None:
    """The r12e executable: $R12E, the release build, or one on $PATH."""
    env = os.environ.get("R12E")
    if env:
        p = Path(env).expanduser()
        return p if p.is_file() and os.access(p, os.X_OK) else None
    built = _REPO / "target" / "release" / "r12e"
    if built.is_file() and os.access(built, os.X_OK):
        return built
    found = shutil.which("r12e")
    return Path(found) if found else None


@register_decompiler("r12e")
class R12eDecompiler(Decompiler):
    """r12e driven through its JSON command line."""
    ...
    def decompile_binary(
        ...
        if self._bin is None:
            raise RuntimeError("r12e is not available: build it with cargo build --release")

        start = time.time()
        # r12e reports a function's virtual address, which for an ELF is already
        ...
        def _meta(partial: bool, error: str = "") -> DecompilerMetadata:
            extra: dict[str, Any] = {"backend": "r12e", "via": "cli"}
            if partial:
                extra["partial"] = True
            if error:
                extra["error"] = error
        ...
        targets_mode = os.environ.get("R12E_MODE") == "targets" and addr_targets
        try:
            if targets_mode:
                items = []
        ...
        for name, addr in narrowed:
            ...
            # r12e counts its own gotos and the constructs it could not model;
            # both say how much of the function it actually structured.
            metadata["r12e_gotos"] = item.get("gotos")
            metadata["r12e_unmodelled"] = item.get("unmodelled")
            metadata["r12e_complete"] = (item.get("function") or {}).get("complete")
    ...
    def _run(self, binary_path: Path, target: str) -> list[dict]:
        """One `r12e decompile` call, returning its JSON items."""
        cmd = [str(self._bin), "decompile", str(binary_path), target, "--json"]
        threads = os.environ.get("R12E_THREADS")
        if threads:
            cmd += ["--threads", threads]
        proc = subprocess.run(
            cmd,
        ...
        if proc.returncode not in (0, 1):
            raise RuntimeError(f"r12e exit {proc.returncode}: {proc.stderr.strip()[:200]}")
        if not proc.stdout.strip():
            return []
        doc = json.loads(proc.stdout)
        if doc.get("schema") != "r12e/1":
